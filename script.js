...
async function dbGet(id) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/quizzes?id=eq.${encodeURIComponent(id)}&select=*`, { headers: HEADERS });
  const data = await res.json();
  return data[0] || null;
}
async function dbList() {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/quizzes?select=id,title,description,created_at,questions&order=created_at.desc`, { headers: HEADERS });
  return await res.json();
}
async function dbSave(quiz) {
  const payload = { id: quiz.id, title: quiz.title, description: quiz.description, questions: quiz.questions };
  const exists = await dbGet(quiz.id);
  if (exists) {
    const updateRes = await fetch(`${SUPABASE_URL}/rest/v1/quizzes?id=eq.${encodeURIComponent(quiz.id)}&select=id`, {
      method: 'PATCH',
      headers: { ...HEADERS, 'Prefer': 'return=representation' },
      body: JSON.stringify(payload)
    });
    if (!updateRes.ok) console.error('Quiz update failed:', updateRes.status, await updateRes.text());
    else if ((await updateRes.json()).length > 0) return { ok: true, copied: false };

    console.warn('Quiz update was not allowed. Saving edited quiz as a new quiz instead.');
    payload.id = genId();
    quiz.id = payload.id;
  }

  const createRes = await fetch(`${SUPABASE_URL}/rest/v1/quizzes`, {
    method: 'POST',
    headers: { ...HEADERS, 'Prefer': 'return=minimal' },
    body: JSON.stringify(payload)
  });
  if (!createRes.ok) console.error('Quiz create failed:', createRes.status, await createRes.text());
  return { ok: createRes.ok, copied: exists };
}
  ...
  return `
    <div class="card">
      ...
      <div class="field" style="margin-bottom:0">
        <label>解説（任意）</label>
        <textarea rows="2" placeholder="解答後に表示される解説を入力してください" oninput="updateExplanation(${qi},this.value)">${esc(q.explanation||'')}</textarea>
      </div>
    </div>`;
}
...
function updateExplanation(qi, val) { currentQuiz.questions[qi].explanation = val; }
...
async function saveQuiz() {
  ...
  showLoading('保存中...');
  const result = await dbSave(currentQuiz);
  const ok = result.ok;
  hideLoading();
...
function renderPlayerQuestions() {
  ...
  document.getElementById('player-questions').innerHTML = currentQuiz.questions.map((q, qi) => {
    const answered = playerAnswers[qi] !== undefined;
    const isCorrect = playerAnswers[qi] === q.correctIndex;
    const choices = q.choices.map((ch, ci) => {
      ...
      return `<button class="${cls}" onclick="selectAnswer(${qi},${ci})" ${answered ? 'disabled' : ''}>
    ...
    if (answered && q.explanation) {
      const expCls = isCorrect ? 'correct-exp' : 'wrong-exp';
      expBox = `<div class="explanation-box show ${expCls}">
    ...
    const resultBadge = answered
      ? (isCorrect ? '<span class="q-result-ok">✓ 正解</span>' : '<span class="q-result-ng">✗ 不正解</span>')
      : '';

    return `<div class="card">
      <div class="q-block-header"><span class="badge">Q${qi+1}</span>${resultBadge}</div>
  ...
  // 全問回答済みならスコアバナーを表示
  if (Object.keys(playerAnswers).length === currentQuiz.questions.length) {
    showScoreBanner();
  }
}

function selectAnswer(qi, ci) {
  if (playerAnswers[qi] !== undefined) return;
  playerAnswers[qi] = ci;
  renderPlayerQuestions();
}
...
function showScoreBanner() {
  ...
  banner.innerHTML = `
    ...
    <div class="score-rate">正答率 ${accuracy}%</div>
...
async function saveCurrentAsNew() {
  ...
  const copiedQuiz = {
    ...
    questions: currentQuiz.questions.map(q => ({
      ...q,
      id: genId(),
      choices: q.choices.map(ch => ({ ...ch, id: genId() }))
    }))
  };

  showLoading('\u4fdd\u5b58\u4e2d...');
  const result = await dbSave(copiedQuiz);
  hideLoading();
  if (!result.ok) {
  ...
  currentQuiz = copiedQuiz;
  sharedId = copiedQuiz.id;
  document.getElementById('share-id-text').textContent = sharedId;
  showPage('share');
  showToast('\u65b0\u3057\u3044ID\u3067\u4fdd\u5b58\u3057\u307e\u3057\u305f');
}
